...
SPEC = LayoutNodeSpec(
    ...
    description=(
        ...
        "3Dmol.js single-geometry viewer. The source input may be a "
        "wf.pointer.v1 JSON string or a concrete XYZ path."
    ),
    entrypoint="scripps_workflow.output_viewers.geometry_bundle:main",
    output_zip=OUTPUT_ZIP,
    inputs=(
        InputSpec("source", required=1, tags="wf.pointer.v1 or xyz path"),
        InputSpec("smiles"),
        InputSpec("title"),
        InputSpec("conformer_index", tags="optional: best or 1-based index"),
